...
def process_line(pipeline, text, keep_pos=1, keep_punct=0, keep_stops=1):
    """
...
def process_unified(line, pipeline, keep_pos, keep_punct, keep_stops):
    line = unify_sym(line.strip())
    line_lems = process_line(pipeline, line, keep_pos, keep_punct, keep_stops)
    return line_lems
